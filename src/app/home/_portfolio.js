...
export default function _portfolio({data=[]}) {
    ...
    return (
        <section id="allportofolio" className="container mb-5 pb-5" >
            ...
            {
                data.map((x,i)=>(
                    <Link key={i} aria-label={x.title} title={x.title} className={x.bg+' '+x.tx +' '+(limitview<i && showall==false?'  d-none ':' ')+' d-block shadow project-item  rounded-5 mb-5 px-5 text-decoration-none'} target="_blank" rel="noreferrer" href={x.url}>
